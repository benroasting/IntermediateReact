...
export default async function MyNotes() {
  ...
  async function fetchNotes() {
    ...
    const from = await db.all(
      "SELECT n.id as id, n.notes as note, f.name ad from_user, t.name to_user FROM notes n JOIN users f on f.id = n.from_user JOIN users t on t.id = n.to_user where from_user = ?",
      ["1"]
    );
    return from;
  }
